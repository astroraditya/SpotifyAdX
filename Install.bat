...
if  %ps-v% LEQ 2 (
    ...
    @echo.
    Echo "https://github.com/astroraditya/SpotifyAdX#possible-problems"
    @echo.
    pause
    exit
)
...
for /f %%i in ('curl --write-out %%{http_code} --silent --output /dev/null --insecure https://raw.githubusercontent.com/astroraditya/SpotifyAdX/main/Install.ps1') do set STATUS_CODE=%%i 2>nul
if %STATUS_CODE% EQU 200 (
    powershell -Command "& {[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12}"; "& {Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/astroraditya/SpotifyAdX/main/Install.ps1' | Invoke-Expression}"
)
